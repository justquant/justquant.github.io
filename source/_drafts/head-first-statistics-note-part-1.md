...
* 正态分布可以用来对二项式分布近似求解：（前面讲过，什么时候可以用泊松分布来近似求解）
    - 如果X ~ B（N，P），那么当np > 5 而且 nq > 5时，可以使用正态分布 X ~ N(np,npq) 来近似求解。（要求pq比较接近0.5）
...
* 分层抽样和巢式抽样的区别：分层抽样是把不同的组看起来很不一样，而巢式抽样是让每一组看起来尽量一样
...
* 根据样本推测总体的均值，往往并不精确，但是却是我们能够得到的最好的估计。
* 点估计量（point estimator): 用于从样本中估算总体的参数的公式。
* 从样本推测整体，要尽量避免样本产生偏差。一般而言，在没有偏差的情况下，样本的总量越大越好
* 平均值估计量：等于样本的平均值。
* 方差估计量：等于样本中所有值减去样本的平均值的平方的和，再除以 n - 1
![方差估计量公式](images/sample_variance_formula.png)
* 使用样本来预测总体的成功率：可以用样本的成功率来近似估算总体的成功率
* 对于二项式分布 Probability = Proportion
* 【问题】：**已知总体分布，求解样本中的分布。** 已知在总体中红色的泡泡糖占25%，求在100个（随机包装的）泡泡糖中红色泡泡糖大于40个的概率。
